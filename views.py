...
class FolderViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            # Return an error response if the folder does not exist
            return Response(
    ...
    def perform_create(self, serializer):
        ...
        #RBAC
        user = self.request.user
        if not hasattr(user, 'permissions') or not user.permissions.can_create_folder:
    ...
    def folder_exists_at_level(self, name, parent=None):
        ...
        name = name.strip()
        return Folder.objects.filter(name=name, parent=parent, created_by=self.request.user._id).exists()
...
# DB_NAME = 'flashcarddb'  # Replace with backup DB name
# RESTORE_DB_NAME = 'Duplicate_Flash'  # New DB name for restore

# @api_view(['POST'])
# @authentication_classes([CustomJWTAuthentication])  # Use your authentication class
# @permission_classes([IsAuthenticated])  # Ensure only authenticated users can access
# def backup_mongodb(request):
#     try:
#         os.makedirs(BACKUP_FOLDER, exist_ok=True)
#         subprocess.run(['mongodump', '--db', DB_NAME, '--out', BACKUP_FOLDER], check=True)
#         return Response({"message": "Backup successful"}, status=status.HTTP_200_OK)
...
#     try:
#         subprocess.run(['mongorestore', '--drop', '--db', RESTORE_DB_NAME, os.path.join(BACKUP_FOLDER, DB_NAME)], check=True)
#         return Response({"message": "Restore successful"}, status=status.HTTP_200_OK)
#     except subprocess.CalledProcessError as e:
#         return Response({"error": "Restore failed", "details": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
#     try:
#         subprocess.run(['mongorestore', '--db', RESTORE_DB_NAME, os.path.join(BACKUP_FOLDER, DB_NAME)], check=True)
#         return Response({"message": "Merge and Restore successful"}, status=status.HTTP_200_OK)
#     except subprocess.CalledProcessError as e:
#         return Response({"error": "Merge and Restore successful", "details": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
# from rest_framework import status
# from rest_framework.permissions import IsAuthenticated
# from .authentication import CustomJWTAuthentication  # Update if needed
...
#         subprocess.run(['mongorestore', '--drop', '--db', restore_db_name, restore_path], check=True)
#         return Response({"message": f"Restore to '{restore_db_name}' successful"}, status=status.HTTP_200_OK)
#     except subprocess.CalledProcessError as e:
#         return Response({"error": "Restore failed", "details": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def user_permission_detail(request, user_id):
    # Only admin or teacher can set permissions
...
def admin_register_user(request):
    ...
    if serializer.is_valid():
        ...
        # Optionally, create default permissions for the new user
...
def initial_admin_register(request):
    ...
    if serializer.is_valid():
        ...
        # Send OTP email to the new admin
